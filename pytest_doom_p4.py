...
def test_doom_p4_boot(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
    dut.expect('Starting Doom P4')
    log_minimum_free_heap_size()


@pytest.mark.host_test
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_doom_p4_linux(dut: IdfDut) -> None:
    dut.expect('Starting Doom P4')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_doom_p4_macos(dut: IdfDut) -> None:
    dut.expect('Starting Doom P4')
...
def test_doom_p4_host(app: QemuApp, dut: QemuDut) -> None:
